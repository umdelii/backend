...
public class SearchBoardRepositoryImpl extends QuerydslRepositorySupport implements SearchBoardRepository {
    ...
    public Page<Object[]> list(String type, String keyword, Pageable pageable) {
        ...
        builder.and(board.bno.gt(0)); // pk가 뭔지 알려주고하는게 빨라서 당연하지만 넣는다

        // type = "t" || "c" || "w" || "tc" || "tcw"
        // tc나 tcw => t, c, w 개별로 찢어야함
        if (type != null) {
        ...
        log.info("===================");
        log.info(query); // select board, member from Board board left join Member member1 with
                         // board.writer = member1

        // select board, member1, count(reply) from Board board left join Member member1
        // with board.writer = member1 left join Reply reply with reply.board =
        // board group by board

        log.info("===================");

        List<Tuple> result = tuple.fetch();
        long count = tuple.fetchCount(); // 전체 행 개수

        // List<Tuple> => List<Object[]>로 변경
        List<Object[]> list = result.stream().map(Tuple::toArray).collect(Collectors.toList());

        // 리턴값 List<Object[]> list, count, pageable 3개
